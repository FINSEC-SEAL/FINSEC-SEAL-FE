...
function ProductApp({ mode, page, go }: { mode: Mode; page: Page; go: (mode: Mode, page: Page) => void }) {
  ...
  return <div className="app-shell">
    ...
    {modeOpen && <Modal title="워크스페이스 환경 설정" onClose={() => setModeOpen(false)}><Notice title={simulated ? '현재: SIMULATED' : '현재: LIVE_API'} tone={simulated ? 'blue' : 'amber'}>{simulated ? '실제 네트워크 요청 없이 모든 UI를 체험합니다. 실제 비밀키·고객정보를 입력하지 마세요.' : '등록·분석·실행·위험 검토·판정·복구와 저장 정책의 검토·검증·승인·거절은 실제 백엔드에 전송됩니다. 정책 작업에는 검토자 브라우저 세션 또는 명시적으로 선택한 로컬 개발 키가 필요합니다.'}</Notice>

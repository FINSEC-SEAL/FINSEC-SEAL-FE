...
export function ExecutionPage({ releases, actorId }: { releases:Release[]; actorId:string }) {
  ...
  return <><PageHeader eyebrow="ATTACK EXECUTION" title="Runs & Trace" description="B Runtime이 수행한 공격·Replay 상태와 Policy, Tool, Oracle 이벤트 체인을 추적합니다." />
    ...
    <section className="panel"><div className="panel-heading"><div><p className="eyebrow">REPLAY COMPARABILITY</p><h2>{replayComparisons.length} replay comparisons</h2></div><span className={`status status--${replayComparisons.some((item) => !item.comparable) ? 'warning' : 'positive'}`}>{replayComparisons.some((item) => !item.comparable) ? 'MISMATCHES PRESENT' : 'COMPARABLE'}</span></div>{replayComparisons.length ? <div className="event-list">{replayComparisons.map((item)=><details key={`${item.baselineRunId}-${item.replayRunId}`}><summary><strong>{item.category ?? 'UNSPECIFIED'}</strong><em>{item.comparable ? 'Comparable replay' : 'Non-comparable replay'}</em><span className={`status status--${item.comparable ? 'positive' : 'warning'}`}>{item.comparable ? 'COMPARABLE' : 'MISMATCH'}</span></summary><div className="execution-form"><label>Baseline run<input readOnly value={item.baselineRunId} /></label><label>Replay run<input readOnly value={item.replayRunId} /></label><button className="secondary-button" type="button" onClick={()=>void openReplayRun(item)}>Replay run 열기</button></div><pre>{JSON.stringify({baselineRunId:item.baselineRunId,replayRunId:item.replayRunId,mismatchReasons:item.mismatchReasons},null,2)}</pre></details>)}</div> : <p className="muted">Replay comparison evidence가 아직 없습니다.</p>}</section>

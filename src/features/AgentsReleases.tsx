    ...
    <section className="release-layout">
      ...
      <div className="release-main">
        ...
        {selected ? <article className="panel release-detail"><div className="panel-heading"><div><p className="eyebrow">SELECTED RELEASE</p><h2>v{selected.version}</h2></div><StatusBadge status={selected.effectiveStatus} /></div>
          ...
          {validation ? <div className={validation.valid ? 'result-box result-box--pass' : 'result-box result-box--fail'}><strong>{validation.valid ? '✓ Manifest valid' : `${validation.issues.length}개 문제`}</strong>{validation.issues.map((issue) => <p key={`${issue.pointer}-${issue.code}`}><code>{issue.pointer}</code> {issue.message}</p>)}</div> : null}

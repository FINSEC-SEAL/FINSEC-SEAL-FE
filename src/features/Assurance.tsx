...
function ReplayPanel({ summary }: { summary?: ReplaySummary | null }) {
  ...
  return <section className="panel replay-panel"><div className="panel-heading"><div><p className="eyebrow">REPLAY COMPARABILITY</p><h2>{summary.evidenceComplete ? 'Replay evidence complete' : 'Review required'}</h2></div><span className={`status status--${summary.evidenceComplete ? 'positive' : 'critical'}`}>{summary.evidenceComplete ? 'COMPLETE' : 'INCOMPLETE'}</span></div>
    ...
    {summary.items.length ? <div className="replay-list">{summary.items.map((item) => <article key={`${item.baselineRunId ?? 'missing'}:${item.replayRunId}`}><div><strong>{item.category ?? 'Replay trial'}</strong><span className={`status status--${item.comparable ? 'positive' : 'critical'}`}>{item.comparable ? 'COMPARABLE' : 'NON-COMPARABLE'}</span></div><code>{item.baselineRunId?.slice(0, 12) ?? 'NO BASELINE'} → {item.replayRunId.slice(0, 12)}</code>{item.mismatchReasons.length ? <ul>{item.mismatchReasons.map((reason) => <li key={reason}>{reason.replaceAll('_', ' ')}</li>)}</ul> : null}</article>)}</div> : <p className="muted">Replay trial이 없습니다.</p>}
  </section>
}

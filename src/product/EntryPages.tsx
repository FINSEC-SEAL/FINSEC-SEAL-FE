...
export function WorkspacePage({ agents, releases, navigate, workflow: w, simulated, start }: { agents: Agent[]; releases: Release[]; navigate: Navigate; workflow: Workflow; simulated: boolean; start: () => void }) {
  return <><PageHeader eyebrow="WORKSPACE" title="검증 워크스페이스" description="에이전트의 검증 상태와 지금 확인할 위험을 한곳에서 봅니다." actions={<button className="primary-button" onClick={start}>샘플 검증 시작 →</button>} />
    ...
    </Panel><Panel title="최근 실행" description={simulated ? 'SIMULATED · run-baseline-demo' : '실제 실행 연동 대기'}>{simulated ? <><Progress label="완료된 trial" value={w.progress} total={40} /><p className="run-current">{w.active ? '현재 단계: 결과 검사' : w.phase === 'cancelled' ? '취소됨 · 부분 증거 보존' : w.hasBaseline ? '기본 실행 완료 · 검토 대기' : '아직 실행하지 않았습니다.'}</p><p className="muted">{w.connection ? '샘플 연결 정상' : '연결 복구 상태 예시'} · 실제 서버 실행 아님</p><button className="secondary-button full-width" onClick={() => navigate('trace')}>실행 추적 열기</button></> : <Notice title="실측값을 임의로 채우지 않습니다.">B·C·D 실행 API가 연결되면 해당 결과를 표시합니다.</Notice>}</Panel></div>
